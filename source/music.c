/* music.c -- BTD5's music path (DroidMusicManager / CustomMediaPlayer).
 *
 * BTD5 has TWO audio paths:
 ...
 *
 * ninjakiwi.c routes the CustomMediaPlayer JNI methods here; the jobject handle
 * we hand back to the engine is simply a Music* cast to a pointer.
 */
#define _GNU_SOURCE
#include <switch.h>
#include <stdlib.h>
...
/* BTD5 asks for names like "main.mp3"; the loose files live under the same
...
void *music_load(const char *path) {
  ...
  if (!enc) {
    debugPrintf("music: load(%s) -> asset MISSING\n", path);
    return NULL;
  }
  ...
  if (rc || !info.buffer || info.samples == 0) {
    debugPrintf("music: load(%s) -> mp3 decode failed rc=%d\n", path, rc);
    free(info.buffer);
    return NULL;
  }
  ...
  debugPrintf("music: load(%s) -> %zu frames @ %d Hz, %d ch\n",
              path, m->frames, m->src_rate, info.channels);
  return m;
}
...
void music_play(void *h, int loop) {
  ...
  SDL_UnlockMutex(g_music_lock);
  debugPrintf("music: play(%p, loop=%d)\n", h, loop);
}
